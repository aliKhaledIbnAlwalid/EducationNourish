...
class _ProfilePageStudentState extends State<ProfilePageStudent> {
  Future<Profile> fetchProfile() async {
    final response = await http.get(Uri.parse('http://54.242.19.19:3000/api/parents'));

    if (response.statusCode == 200) {
      print('API Response: ${response.body}'); // ✅ Optional: log the response

      List<dynamic> data = json.decode(response.body);

      if (data.isNotEmpty && data[0] is Map<String, dynamic>) {
        return Profile.fromJson(data[0]); // ✅ get the first profile
      } else {
                        ...
                        child: Padding(
                          ...
                          child: Column(
                            children: [
                              ...
                              InfoRow(label: 'address', value: profile.address),
                              InfoRow(label: 'phoneNumber', value: profile.phoneNumber),
                              InfoRow(label: 'gender', value: profile.gender),
                              InfoRow(label: 'email', value: profile.email),
                            ],
                          ),
